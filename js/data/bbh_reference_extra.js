// GENERATED FILE — do not edit. Regenerate: node tools/gen_bbh_reference_extra.mjs
// (sources: source/bbh/alphabet.json, source/bbh/vowels.json, source/bbh/parsing/paradigms.json)
// Expanded reference tables (alphabet/vowel charts + paradigm tables) for the
// Reference page. Self-registers on window.BBH_REFERENCE_EXTRA, a sibling of
// window.BBH_REFERENCE (js/data/bbh_reference_data.js) — see the header comment
// in tools/gen_bbh_reference_extra.mjs for why this is a separate file.

(function () {
  window.BBH_REFERENCE_EXTRA = {
    "schemaVersion": 1,
    "sections": [
      {
        "id": "alphabet-chart",
        "title": "Hebrew Alphabet (23 Letters)",
        "columns": [
          "Letter",
          "Final",
          "Name (Hebrew)",
          "Name (English)",
          "Sound",
          "Begadkefat",
          "Guttural"
        ],
        "rows": [
          {
            "id": "alphabet-chart-1",
            "cells": [
              "א",
              "—",
              "אָלֶף",
              "alef",
              "glottal stop or silent",
              "No",
              "Yes"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-2",
            "cells": [
              "ב",
              "—",
              "בֵּית",
              "bet",
              "b in Boy (with dagesh); v in Voice (without)",
              "Yes",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-3",
            "cells": [
              "ג",
              "—",
              "גִּימֶל",
              "gimel",
              "g in Give (same with or without dagesh)",
              "Yes",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-4",
            "cells": [
              "ד",
              "—",
              "דָּלֶת",
              "dalet",
              "d in Dog (same with or without dagesh)",
              "Yes",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-5",
            "cells": [
              "ה",
              "—",
              "הֵא",
              "he",
              "h in Hat",
              "No",
              "Yes"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-6",
            "cells": [
              "ו",
              "—",
              "וָו",
              "vav",
              "v in Voice",
              "No",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-7",
            "cells": [
              "ז",
              "—",
              "זַיִן",
              "zayin",
              "z in Zip",
              "No",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-8",
            "cells": [
              "ח",
              "—",
              "חֵית",
              "chet",
              "ch in BaCH",
              "No",
              "Yes"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-9",
            "cells": [
              "ט",
              "—",
              "טֵית",
              "tet",
              "t in Tide",
              "No",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-10",
            "cells": [
              "י",
              "—",
              "יוֹד",
              "yod",
              "y in Yellow",
              "No",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-11",
            "cells": [
              "כ",
              "ך",
              "כַּף",
              "kaf",
              "k in Keep (with dagesh); ch in BaCH (without dagesh, incl. final ך)",
              "Yes",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-12",
            "cells": [
              "ל",
              "—",
              "לָמֶד",
              "lamed",
              "l in Letter",
              "No",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-13",
            "cells": [
              "מ",
              "ם",
              "מֵם",
              "mem",
              "m in Mother",
              "No",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-14",
            "cells": [
              "נ",
              "ן",
              "נוּן",
              "nun",
              "n in Noon",
              "No",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-15",
            "cells": [
              "ס",
              "—",
              "סָמֶךְ",
              "samech",
              "s in Sit",
              "No",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-16",
            "cells": [
              "ע",
              "—",
              "עַיִן",
              "ayin",
              "pharyngeal or silent",
              "No",
              "Yes"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-17",
            "cells": [
              "פ",
              "ף",
              "פֵּא",
              "pe",
              "p in Pie (with dagesh); f in Fish (without dagesh, incl. final ף)",
              "Yes",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-18",
            "cells": [
              "צ",
              "ץ",
              "צָדֵי",
              "tsade",
              "ts in caTS",
              "No",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-19",
            "cells": [
              "ק",
              "—",
              "קוֹף",
              "qof",
              "k in Keep",
              "No",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-20",
            "cells": [
              "ר",
              "—",
              "רֵישׁ",
              "resh",
              "r in Race",
              "No",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "alphabet-chart-21",
            "cells": [
              "שׂ",
              "—",
              "שִׂין",
              "sin",
              "s in Sit",
              "No",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": "Book gives sin (dot on left arm) its own row, distinct from shin, both counted among the 23 consonants."
          },
          {
            "id": "alphabet-chart-22",
            "cells": [
              "שׁ",
              "—",
              "שִׁין",
              "shin",
              "sh in SHin",
              "No",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": "Book gives shin (dot on right arm) its own row, distinct from sin, both counted among the 23 consonants."
          },
          {
            "id": "alphabet-chart-23",
            "cells": [
              "ת",
              "—",
              "תָּו",
              "tav",
              "t in Tide (same with or without dagesh)",
              "Yes",
              "No"
            ],
            "introducedLesson": 1,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 1,
            "page": "22-23",
            "pageCount": 2
          }
        ],
        "introducedLesson": 1
      },
      {
        "id": "vowel-chart",
        "title": "Vowel Points (Niqqud) + Shva Rules",
        "columns": [
          "Sign",
          "Name (Hebrew)",
          "Name (English)",
          "Sound class",
          "Length",
          "Sound",
          "Mater"
        ],
        "rows": [
          {
            "id": "vowel-chart-1",
            "cells": [
              "בַ",
              "פָּתַח",
              "pátach",
              "a",
              "short",
              "a in Father",
              "—"
            ],
            "introducedLesson": 2,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "vowel-chart-2",
            "cells": [
              "בָ / בָה",
              "קָמֶץ(־הֵא)",
              "qamets(-he)",
              "a",
              "lengthened short (bare form) / pure long (mater-he form) — book's chart gives one name/pronunciation for both forms; length terms per Appendix A 2a",
              "a in Father",
              "ה"
            ],
            "introducedLesson": 2,
            "appendixOnly": false,
            "ambiguityNote": "Row covers two forms sharing one name and pronunciation: bare קָמֶץ (ב) and word-final mater-lectionis spelling קָמֶץ־הֵא (בָה); the ה mater is the optional/alternate spelling shown in parentheses."
          },
          {
            "id": "vowel-chart-3",
            "cells": [
              "בֶ",
              "סְגוֹל",
              "segol",
              "e",
              "short",
              "e in Met",
              "—"
            ],
            "introducedLesson": 2,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "vowel-chart-4",
            "cells": [
              "בֵ / בֵי",
              "צֵרֵי(־יוֹד)",
              "tsére(-yod)",
              "e",
              "lengthened short (bare form) / pure long (mater-yod form) — per Appendix A 2a",
              "ey in They",
              "י"
            ],
            "introducedLesson": 2,
            "appendixOnly": false,
            "ambiguityNote": "Row covers two forms sharing one name and pronunciation: bare צֵרֵי (ב) and mater-lectionis spelling צֵרֵי־יוֹד (בֵי)."
          },
          {
            "id": "vowel-chart-5",
            "cells": [
              "בִ / בִי",
              "חִירֶק(־יוֹד)",
              "chireq(-yod)",
              "i",
              "short (bare form) / pure long (mater-yod form) — per Appendix A 2a",
              "i in Machine",
              "י"
            ],
            "introducedLesson": 2,
            "appendixOnly": false,
            "ambiguityNote": "Row covers two forms sharing one name and pronunciation: bare חִירֶק (ב) and mater-lectionis spelling חִירֶק־יוֹד (בִי)."
          },
          {
            "id": "vowel-chart-6",
            "cells": [
              "בָ",
              "קָמֶץ־חָטוּף",
              "qamets-chatuf",
              "o",
              "short",
              "o in Go",
              "—"
            ],
            "introducedLesson": 2,
            "appendixOnly": false,
            "ambiguityNote": "Appendix A a-6 states explicitly this same sign represents the short u-class קָמֶץ חָטוּף in a closed, unstressed syllable (and elsewhere the lengthened a-class קָמֶץ)."
          },
          {
            "id": "vowel-chart-7",
            "cells": [
              "בֹ / בוֹ",
              "חוֹלֶם(־וָו)",
              "chôlem(-vav)",
              "o",
              "lengthened short (bare form) / pure long (mater-vav form) — per Appendix A 2a",
              "o in Go",
              "ו"
            ],
            "introducedLesson": 2,
            "appendixOnly": false,
            "ambiguityNote": "Row covers two forms sharing one name and pronunciation: bare חוֹלֶם (ב) and mater-lectionis spelling חוֹלֶם־וָו (בוֹ)."
          },
          {
            "id": "vowel-chart-8",
            "cells": [
              "בֻ",
              "קִבּוּץ",
              "qibbuts",
              "u",
              "short",
              "u in Rude",
              "—"
            ],
            "introducedLesson": 2,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "vowel-chart-9",
            "cells": [
              "בוּ",
              "שׁוּרֶק",
              "shúreq",
              "u",
              "pure long",
              "u in Rude",
              "ו"
            ],
            "introducedLesson": 2,
            "appendixOnly": false,
            "ambiguityNote": "Always written with the ו mater (there is no separate bare form named in the book's chart)."
          },
          {
            "id": "vowel-chart-10",
            "cells": [
              "בֲ",
              "חֲטֶף־פַּתַח",
              "chatef-pátach",
              "a",
              "reduced short",
              "a in Arise",
              "—"
            ],
            "introducedLesson": 3,
            "appendixOnly": false,
            "ambiguityNote": "Composite/chatef shəva: a combination of the שְׁוָא and one of the three basic vowels, used to give gutturals a bit more sound in place of a plain vocal שְׁוָא. Appendix A 2a lists this sign in the Reduced Short column for the a-class."
          },
          {
            "id": "vowel-chart-11",
            "cells": [
              "בֱ",
              "חֲטֶף־סְגוֹל",
              "chatef-segól",
              "e",
              "reduced short",
              "e in Excuse",
              "—"
            ],
            "introducedLesson": 3,
            "appendixOnly": false,
            "ambiguityNote": "Composite/chatef shəva under gutturals; Appendix A 2a lists this sign in the Reduced Short column for the i-class."
          },
          {
            "id": "vowel-chart-12",
            "cells": [
              "בֳ",
              "חֲטֶף־קָמֶץ",
              "chatef-qamets",
              "o",
              "reduced short",
              "o in Omit",
              "—"
            ],
            "introducedLesson": 3,
            "appendixOnly": false,
            "ambiguityNote": "Composite/chatef shəva under gutturals; Appendix A 2a lists this sign in the Reduced Short column for the u-class."
          }
        ],
        "sourceRef": [
          {
            "lesson": 2,
            "page": 26
          },
          {
            "lesson": 3,
            "page": 29
          }
        ],
        "footnotes": [
          {
            "id": "vowel-chart-sheva-rule-1",
            "text": "The שְׁוָא is vocal (has the 'hurried' a-as-in-above sound, transliterated ə) at the beginning of a word, e.g. שְׁמוֹ [shə-mo].",
            "source": {
              "lesson": 3,
              "page": 29
            }
          },
          {
            "id": "vowel-chart-sheva-rule-2",
            "text": "The שְׁוָא is vocal when it is the second of two consecutive shevas, e.g. the מְ in יִשְׁמְרוּ [yish-mə-ru].",
            "source": {
              "lesson": 3,
              "page": 29
            }
          },
          {
            "id": "vowel-chart-sheva-rule-3",
            "text": "In all other cases, the שְׁוָא is silent, e.g. as under the ד in מִדְבָּר [mid-bar].",
            "source": {
              "lesson": 3,
              "page": 29
            }
          },
          {
            "id": "vowel-chart-sheva-rule-4",
            "text": "Under the guttural consonants, the שְׁוָא is often modified so that it has a bit more sound (e.g. אֲשֶׁר instead of the unmodified *אְשֶׁר). This modified type is called חֲטֶף שְׁוָא (abbreviated as שְׁוָא) and is a combination of the שְׁוָא and one of the three basic vowels (patach, segol, or qamets).",
            "source": {
              "lesson": 3,
              "page": 29
            }
          }
        ],
        "introducedLesson": 2
      },
      {
        "id": "pron-subject",
        "title": "Independent subject pronouns",
        "columns": [
          "Person",
          "Gender",
          "Number",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "pron-subject-3ms",
            "cells": [
              "3",
              "Masculine",
              "Singular",
              "הוּא",
              ""
            ],
            "introducedLesson": 5,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "pron-subject-3fs",
            "cells": [
              "3",
              "Feminine",
              "Singular",
              "הִיא",
              ""
            ],
            "introducedLesson": 5,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "pron-subject-2ms",
            "cells": [
              "2",
              "Masculine",
              "Singular",
              "אַתָּה",
              ""
            ],
            "introducedLesson": 5,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "pron-subject-2fs",
            "cells": [
              "2",
              "Feminine",
              "Singular",
              "אַתְּ",
              ""
            ],
            "introducedLesson": 5,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "pron-subject-1cs",
            "cells": [
              "1",
              "Common",
              "Singular",
              "אֲנִי",
              ""
            ],
            "introducedLesson": 5,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "pron-subject-3mp",
            "cells": [
              "3",
              "Masculine",
              "Plural",
              "הֵם",
              ""
            ],
            "introducedLesson": 11,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "pron-subject-3fp",
            "cells": [
              "3",
              "Feminine",
              "Plural",
              "הֵן",
              ""
            ],
            "introducedLesson": 11,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "pron-subject-2mp",
            "cells": [
              "2",
              "Masculine",
              "Plural",
              "אַתֶּם",
              ""
            ],
            "introducedLesson": 11,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "pron-subject-2fp",
            "cells": [
              "2",
              "Feminine",
              "Plural",
              "אַתֶּן",
              ""
            ],
            "introducedLesson": 11,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "pron-subject-1cp",
            "cells": [
              "1",
              "Common",
              "Plural",
              "אֲנַחְנוּ",
              ""
            ],
            "introducedLesson": 11,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "pron-subject-1cs-alt",
            "cells": [
              "1",
              "Common",
              "Singular",
              "אָנֹכִי",
              "alternate 'I'; appendix pairs it with אֲנִי but only אֲנִי appears in the L5 lesson chart"
            ],
            "introducedLesson": 5,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "pron-subject-3mp-alt",
            "cells": [
              "3",
              "Masculine",
              "Plural",
              "הֵמָּה",
              "longer alt. 3mp form; identical spelling to far-demonstrative plural (pron-demonstrative-far-mp)"
            ],
            "introducedLesson": 11,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "pron-subject-3fp-alt",
            "cells": [
              "3",
              "Feminine",
              "Plural",
              "הֵנָּה",
              "longer alt. 3fp form; identical spelling to far-demonstrative plural (pron-demonstrative-far-fp)"
            ],
            "introducedLesson": 11,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 5,
            "page": 32
          },
          {
            "lesson": 11,
            "page": 44
          },
          {
            "appendix": "B",
            "page": "a-12"
          }
        ],
        "introducedLesson": 5
      },
      {
        "id": "particle-interrogative-words",
        "title": "Question words (interrogatives)",
        "columns": [
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "particle-interrogative-mi",
            "cells": [
              "מִי",
              "who?"
            ],
            "introducedLesson": 6,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-interrogative-mah",
            "cells": [
              "מָה",
              "what?"
            ],
            "introducedLesson": 8,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-interrogative-lammah",
            "cells": [
              "לָמָּה",
              "why?"
            ],
            "introducedLesson": 14,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-interrogative-maddua",
            "cells": [
              "מַדּוּעַ",
              "why? (alternate)"
            ],
            "introducedLesson": 14,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-interrogative-bammeh",
            "cells": [
              "בַּמֶּה",
              "how? (with preposition ב)"
            ],
            "introducedLesson": 14,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-interrogative-eykh",
            "cells": [
              "אֵיךְ",
              "how? (alternate)"
            ],
            "introducedLesson": 14,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-interrogative-kammah",
            "cells": [
              "כַּמָּה",
              "how much/how many?"
            ],
            "introducedLesson": 14,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-interrogative-matay",
            "cells": [
              "מָתַי",
              "when?"
            ],
            "introducedLesson": 14,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-interrogative-ey",
            "cells": [
              "אֵי",
              "where? (short form)"
            ],
            "introducedLesson": 14,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-interrogative-ayyeh",
            "cells": [
              "אַיֵּה",
              "where? (alternate)"
            ],
            "introducedLesson": 14,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-interrogative-eyfoh",
            "cells": [
              "אֵיפֹה",
              "where? (alternate)"
            ],
            "introducedLesson": 14,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-interrogative-meayin",
            "cells": [
              "מֵאַיִן",
              "from where?"
            ],
            "introducedLesson": 14,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-interrogative-an",
            "cells": [
              "אָן",
              "where?/to where? (short form)"
            ],
            "introducedLesson": 14,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-interrogative-anah",
            "cells": [
              "אָנָה",
              "where?/to where? (alternate)"
            ],
            "introducedLesson": 14,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 6,
            "page": 34
          },
          {
            "lesson": 8,
            "page": 38
          },
          {
            "lesson": 14,
            "page": 51
          }
        ],
        "introducedLesson": 6
      },
      {
        "id": "noun-endings",
        "title": "Noun gender/number/dual endings (סוּס/סוּסָה pattern)",
        "columns": [
          "Word",
          "Gender",
          "Number",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "noun-endings-m-sg",
            "cells": [
              "סוּס",
              "Masculine",
              "Singular",
              "סוּס",
              "stallion; masculine singular has no inflectional ending"
            ],
            "introducedLesson": 7,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-endings-f-sg",
            "cells": [
              "סוּסָה",
              "Feminine",
              "Singular",
              "סוּסָה",
              "mare; ־ָה feminine singular ending"
            ],
            "introducedLesson": 7,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-endings-m-pl",
            "cells": [
              "סוּס",
              "Masculine",
              "Plural",
              "סוּסִים",
              "stallions"
            ],
            "introducedLesson": 10,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-endings-f-pl",
            "cells": [
              "סוּסָה",
              "Feminine",
              "Plural",
              "סוּסוֹת",
              "mares"
            ],
            "introducedLesson": 10,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-endings-m-dual",
            "cells": [
              "סוּס",
              "Masculine",
              "Dual",
              "סוּסַיִם",
              "dual ־ַיִם; used for naturally-paired items"
            ],
            "introducedLesson": 10,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-endings-f-dual",
            "cells": [
              "סוּסָה",
              "Feminine",
              "Dual",
              "סוּסָתַיִם",
              ""
            ],
            "introducedLesson": 10,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 7,
            "page": 36
          },
          {
            "lesson": 10,
            "page": 41
          }
        ],
        "introducedLesson": 7
      },
      {
        "id": "noun-irregular",
        "title": "Frequent irregular nouns: absolute and construct, singular and plural",
        "columns": [
          "Word",
          "Gender",
          "Number",
          "State",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "noun-irregular-av-abs-sg",
            "cells": [
              "אָב",
              "Masculine",
              "Singular",
              "Absolute",
              "אָב",
              "father"
            ],
            "introducedLesson": 7,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-av-abs-pl",
            "cells": [
              "אָב",
              "Masculine",
              "Plural",
              "Absolute",
              "אָבוֹת",
              "fathers"
            ],
            "introducedLesson": 7,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-av-cst-sg",
            "cells": [
              "אָב",
              "Masculine",
              "Singular",
              "Construct",
              "אֲבִי",
              "construct sg.; appendix also lists a variant אַב"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-av-cst-pl",
            "cells": [
              "אָב",
              "Masculine",
              "Plural",
              "Construct",
              "אֲבוֹת",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-ach-abs-sg",
            "cells": [
              "אָח",
              "Masculine",
              "Singular",
              "Absolute",
              "אָח",
              "brother"
            ],
            "introducedLesson": 10,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-ach-abs-pl",
            "cells": [
              "אָח",
              "Masculine",
              "Plural",
              "Absolute",
              "אַחִים",
              "brothers"
            ],
            "introducedLesson": 10,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-ach-cst-sg",
            "cells": [
              "אָח",
              "Masculine",
              "Singular",
              "Construct",
              "אֲחִי",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-ach-cst-pl",
            "cells": [
              "אָח",
              "Masculine",
              "Plural",
              "Construct",
              "אֲחֵי",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-achot-abs-sg",
            "cells": [
              "אָחוֹת",
              "Feminine",
              "Singular",
              "Absolute",
              "אָחוֹת",
              "sister"
            ],
            "introducedLesson": 10,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-achot-abs-pl",
            "cells": [
              "אָחוֹת",
              "Feminine",
              "Plural",
              "Absolute",
              "אַחְיוֹת",
              "sisters"
            ],
            "introducedLesson": 10,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-achot-cst-sg",
            "cells": [
              "אָחוֹת",
              "Feminine",
              "Singular",
              "Construct",
              "אֲחוֹת",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-achot-cst-pl",
            "cells": [
              "אָחוֹת",
              "Feminine",
              "Plural",
              "Construct",
              "אַחְיוֹת",
              "appendix prints this identical in spelling to the absolute plural"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-ish-abs-sg",
            "cells": [
              "אִישׁ",
              "Masculine",
              "Singular",
              "Absolute",
              "אִישׁ",
              "man"
            ],
            "introducedLesson": 10,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-ish-abs-pl",
            "cells": [
              "אִישׁ",
              "Masculine",
              "Plural",
              "Absolute",
              "אֲנָשִׁים",
              "men; suppletive plural"
            ],
            "introducedLesson": 10,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-ish-cst-sg",
            "cells": [
              "אִישׁ",
              "Masculine",
              "Singular",
              "Construct",
              "אִישׁ",
              "construct sg. identical in spelling to absolute"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-ish-cst-pl",
            "cells": [
              "אִישׁ",
              "Masculine",
              "Plural",
              "Construct",
              "אַנְשֵׁי",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-isha-abs-sg",
            "cells": [
              "אִשָּׁה",
              "Feminine",
              "Singular",
              "Absolute",
              "אִשָּׁה",
              "woman"
            ],
            "introducedLesson": 10,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-isha-abs-pl",
            "cells": [
              "אִשָּׁה",
              "Feminine",
              "Plural",
              "Absolute",
              "נָשִׁים",
              "women; suppletive plural"
            ],
            "introducedLesson": 10,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-isha-cst-sg",
            "cells": [
              "אִשָּׁה",
              "Feminine",
              "Singular",
              "Construct",
              "אֵשֶׁת",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-isha-cst-pl",
            "cells": [
              "אִשָּׁה",
              "Feminine",
              "Plural",
              "Construct",
              "נְשֵׁי",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-ben-abs-sg",
            "cells": [
              "בֵּן",
              "Masculine",
              "Singular",
              "Absolute",
              "בֵּן",
              "son"
            ],
            "introducedLesson": 7,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-ben-abs-pl",
            "cells": [
              "בֵּן",
              "Masculine",
              "Plural",
              "Absolute",
              "בָּנִים",
              "sons"
            ],
            "introducedLesson": 7,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-ben-cst-sg",
            "cells": [
              "בֵּן",
              "Masculine",
              "Singular",
              "Construct",
              "בֶּן",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-ben-cst-pl",
            "cells": [
              "בֵּן",
              "Masculine",
              "Plural",
              "Construct",
              "בְּנֵי",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-bat-abs-sg",
            "cells": [
              "בַּת",
              "Feminine",
              "Singular",
              "Absolute",
              "בַּת",
              "daughter"
            ],
            "introducedLesson": 7,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-bat-abs-pl",
            "cells": [
              "בַּת",
              "Feminine",
              "Plural",
              "Absolute",
              "בָּנוֹת",
              "daughters"
            ],
            "introducedLesson": 7,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-bat-cst-sg",
            "cells": [
              "בַּת",
              "Feminine",
              "Singular",
              "Construct",
              "בַּת",
              "construct sg. identical in spelling to absolute"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-bat-cst-pl",
            "cells": [
              "בַּת",
              "Feminine",
              "Plural",
              "Construct",
              "בְּנוֹת",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-rosh-abs-sg",
            "cells": [
              "רֹאשׁ",
              "Masculine",
              "Singular",
              "Absolute",
              "רֹאשׁ",
              "head"
            ],
            "introducedLesson": 10,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-rosh-abs-pl",
            "cells": [
              "רֹאשׁ",
              "Masculine",
              "Plural",
              "Absolute",
              "רָאשִׁים",
              "heads"
            ],
            "introducedLesson": 10,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-rosh-cst-sg",
            "cells": [
              "רֹאשׁ",
              "Masculine",
              "Singular",
              "Construct",
              "רֹאשׁ",
              "construct sg. identical in spelling to absolute"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-rosh-cst-pl",
            "cells": [
              "רֹאשׁ",
              "Masculine",
              "Plural",
              "Construct",
              "רָאשֵׁי",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 7,
            "page": 37
          },
          {
            "lesson": 10,
            "page": "41-42",
            "pageCount": 2
          },
          {
            "appendix": "B",
            "page": "a-11-a-12",
            "pageCount": 2
          }
        ],
        "introducedLesson": 7
      },
      {
        "id": "noun-irregular-bayit",
        "title": "בַּיִת 'house' (irregular absolute/construct, sg/pl)",
        "columns": [
          "Number",
          "State",
          "Hebrew"
        ],
        "rows": [
          {
            "id": "noun-irregular-bayit-abs-sg",
            "cells": [
              "Singular",
              "Absolute",
              "בַּיִת"
            ],
            "introducedLesson": 8,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-bayit-cons-sg",
            "cells": [
              "Singular",
              "Construct",
              "בֵּית"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-bayit-abs-pl",
            "cells": [
              "Plural",
              "Absolute",
              "בָּתִּים"
            ],
            "introducedLesson": 8,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-bayit-cons-pl",
            "cells": [
              "Plural",
              "Construct",
              "בָּתֵּי"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 8,
            "page": 38
          },
          {
            "appendix": "B",
            "page": "a-11"
          }
        ],
        "introducedLesson": 8
      },
      {
        "id": "particle-l-possession",
        "title": "ל of possession + attached pronoun",
        "columns": [
          "Suffix",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "particle-l-possession-3ms",
            "cells": [
              "3ms",
              "לוֹ",
              "'belongs to him/it'"
            ],
            "introducedLesson": 9,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-l-possession-3fs",
            "cells": [
              "3fs",
              "לָהּ",
              ""
            ],
            "introducedLesson": 9,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-l-possession-2ms",
            "cells": [
              "2ms",
              "לְךָ",
              ""
            ],
            "introducedLesson": 9,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-l-possession-2fs",
            "cells": [
              "2fs",
              "לָךְ",
              ""
            ],
            "introducedLesson": 9,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-l-possession-1cs",
            "cells": [
              "1cs",
              "לִי",
              ""
            ],
            "introducedLesson": 9,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-l-possession-3mp",
            "cells": [
              "3mp",
              "לָהֶם",
              ""
            ],
            "introducedLesson": 17,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-l-possession-3fp",
            "cells": [
              "3fp",
              "לָהֶן",
              ""
            ],
            "introducedLesson": 17,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-l-possession-2mp",
            "cells": [
              "2mp",
              "לָכֶם",
              ""
            ],
            "introducedLesson": 17,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-l-possession-2fp",
            "cells": [
              "2fp",
              "לָכֶן",
              ""
            ],
            "introducedLesson": 17,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "particle-l-possession-1cp",
            "cells": [
              "1cp",
              "לָנוּ",
              ""
            ],
            "introducedLesson": 17,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 9,
            "page": 39
          },
          {
            "lesson": 17,
            "page": 58
          }
        ],
        "introducedLesson": 9
      },
      {
        "id": "noun-irregular-ir",
        "title": "עִיר 'city' (irregular absolute/construct, sg/pl)",
        "columns": [
          "Number",
          "State",
          "Hebrew"
        ],
        "rows": [
          {
            "id": "noun-irregular-ir-abs-sg",
            "cells": [
              "Singular",
              "Absolute",
              "עִיר"
            ],
            "introducedLesson": 11,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-ir-cons-sg",
            "cells": [
              "Singular",
              "Construct",
              "עִיר"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": "Construct singular is spelled identically to the absolute singular; disambiguate by whether the noun heads a construct phrase (followed by another noun/def. article target)."
          },
          {
            "id": "noun-irregular-ir-abs-pl",
            "cells": [
              "Plural",
              "Absolute",
              "עָרִים"
            ],
            "introducedLesson": 11,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-ir-cons-pl",
            "cells": [
              "Plural",
              "Construct",
              "עָרֵי"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 11,
            "page": 44
          },
          {
            "appendix": "B",
            "page": "a-12"
          }
        ],
        "introducedLesson": 11
      },
      {
        "id": "verb-qal-perfect",
        "title": "Qal Perfect (שמר)",
        "columns": [
          "Person",
          "Gender",
          "Number",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "verb-qal-perfect-3ms",
            "cells": [
              "3",
              "Masculine",
              "Singular",
              "שָׁמַר",
              "guard; no person/gender/number suffix (bare 3ms form)"
            ],
            "introducedLesson": 16,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-perfect-3fs",
            "cells": [
              "3",
              "Feminine",
              "Singular",
              "שָׁמְרָה",
              ""
            ],
            "introducedLesson": 16,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-perfect-2ms",
            "cells": [
              "2",
              "Masculine",
              "Singular",
              "שָׁמַרְתָּ",
              ""
            ],
            "introducedLesson": 16,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-perfect-2fs",
            "cells": [
              "2",
              "Feminine",
              "Singular",
              "שָׁמַרְתְּ",
              ""
            ],
            "introducedLesson": 16,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-perfect-1cs",
            "cells": [
              "1",
              "Common",
              "Singular",
              "שָׁמַרְתִּי",
              ""
            ],
            "introducedLesson": 16,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-perfect-3cp",
            "cells": [
              "3",
              "Common",
              "Plural",
              "שָׁמְרוּ",
              "3cp is common gender (no masc/fem split in Perfect plural 3rd person)"
            ],
            "introducedLesson": 19,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-perfect-2mp",
            "cells": [
              "2",
              "Masculine",
              "Plural",
              "שְׁמַרְתֶּם",
              ""
            ],
            "introducedLesson": 19,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-perfect-2fp",
            "cells": [
              "2",
              "Feminine",
              "Plural",
              "שְׁמַרְתֶּן",
              ""
            ],
            "introducedLesson": 19,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-perfect-1cp",
            "cells": [
              "1",
              "Common",
              "Plural",
              "שָׁמַרְנוּ",
              ""
            ],
            "introducedLesson": 19,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 16,
            "page": 55
          },
          {
            "lesson": 19,
            "page": 62
          }
        ],
        "introducedLesson": 16
      },
      {
        "id": "verb-haya-perfect",
        "title": "Qal Perfect of היה (irregular \"to be\")",
        "columns": [
          "Person",
          "Gender",
          "Number",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "verb-haya-perfect-3ms",
            "cells": [
              "3",
              "Masculine",
              "Singular",
              "הָיָה",
              "be/become/happen; irregular, taught alongside שמר as the frequent exception"
            ],
            "introducedLesson": 16,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-haya-perfect-3fs",
            "cells": [
              "3",
              "Feminine",
              "Singular",
              "הָיְתָה",
              ""
            ],
            "introducedLesson": 16,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-haya-perfect-2ms",
            "cells": [
              "2",
              "Masculine",
              "Singular",
              "הָיִיתָ",
              ""
            ],
            "introducedLesson": 16,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-haya-perfect-2fs",
            "cells": [
              "2",
              "Feminine",
              "Singular",
              "הָיִית",
              ""
            ],
            "introducedLesson": 16,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-haya-perfect-1cs",
            "cells": [
              "1",
              "Common",
              "Singular",
              "הָיִיתִי",
              ""
            ],
            "introducedLesson": 16,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-haya-perfect-3cp",
            "cells": [
              "3",
              "Common",
              "Plural",
              "הָיוּ",
              ""
            ],
            "introducedLesson": 19,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-haya-perfect-2mp",
            "cells": [
              "2",
              "Masculine",
              "Plural",
              "הֱיִיתֶם",
              ""
            ],
            "introducedLesson": 19,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-haya-perfect-2fp",
            "cells": [
              "2",
              "Feminine",
              "Plural",
              "הֱיִיתֶן",
              ""
            ],
            "introducedLesson": 19,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-haya-perfect-1cp",
            "cells": [
              "1",
              "Common",
              "Plural",
              "הָיִינוּ",
              ""
            ],
            "introducedLesson": 19,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 16,
            "page": 55
          },
          {
            "lesson": 19,
            "page": 62
          }
        ],
        "introducedLesson": 16
      },
      {
        "id": "noun-bound-construct",
        "title": "Bound (construct/נסמך) vs. absolute (סומך) noun state",
        "columns": [
          "Word",
          "Gender",
          "Number",
          "State",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "noun-bound-construct-m-abs-sg",
            "cells": [
              "דָּבָר",
              "Masculine",
              "Singular",
              "Absolute",
              "דָּבָר",
              "word"
            ],
            "introducedLesson": 20,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-bound-construct-m-cst-sg",
            "cells": [
              "דָּבָר",
              "Masculine",
              "Singular",
              "Construct",
              "דְּבַר",
              "word of"
            ],
            "introducedLesson": 20,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-bound-construct-m-abs-pl",
            "cells": [
              "דָּבָר",
              "Masculine",
              "Plural",
              "Absolute",
              "דְּבָרִים",
              "words"
            ],
            "introducedLesson": 20,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-bound-construct-m-cst-pl",
            "cells": [
              "דָּבָר",
              "Masculine",
              "Plural",
              "Construct",
              "דִּבְרֵי",
              "words of"
            ],
            "introducedLesson": 20,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-bound-construct-f-abs-sg",
            "cells": [
              "אֲדָמָה",
              "Feminine",
              "Singular",
              "Absolute",
              "אֲדָמָה",
              "ground"
            ],
            "introducedLesson": 20,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-bound-construct-f-cst-sg",
            "cells": [
              "אֲדָמָה",
              "Feminine",
              "Singular",
              "Construct",
              "אַדְמַת",
              "ground of"
            ],
            "introducedLesson": 20,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-bound-construct-f-abs-pl",
            "cells": [
              "אֲדָמָה",
              "Feminine",
              "Plural",
              "Absolute",
              "אֲדָמוֹת",
              "grounds"
            ],
            "introducedLesson": 20,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-bound-construct-f-cst-pl",
            "cells": [
              "אֲדָמָה",
              "Feminine",
              "Plural",
              "Construct",
              "אַדְמוֹת",
              "construct plural looks identical in the printed chart to the absolute plural for this noun"
            ],
            "introducedLesson": 20,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 20,
            "page": 64
          }
        ],
        "introducedLesson": 20
      },
      {
        "id": "noun-irregular-yom",
        "title": "יוֹם 'day' (irregular absolute/construct, sg/pl)",
        "columns": [
          "Number",
          "State",
          "Hebrew"
        ],
        "rows": [
          {
            "id": "noun-irregular-yom-abs-sg",
            "cells": [
              "Singular",
              "Absolute",
              "יוֹם"
            ],
            "introducedLesson": 21,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-yom-cons-sg",
            "cells": [
              "Singular",
              "Construct",
              "יוֹם"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": "Construct singular is spelled identically to the absolute singular; disambiguate by whether the noun heads a construct phrase (followed by another noun/def. article target)."
          },
          {
            "id": "noun-irregular-yom-abs-pl",
            "cells": [
              "Plural",
              "Absolute",
              "יָמִים"
            ],
            "introducedLesson": 21,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-irregular-yom-cons-pl",
            "cells": [
              "Plural",
              "Construct",
              "יְמֵי"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 21,
            "page": 67
          },
          {
            "appendix": "B",
            "page": "a-12"
          }
        ],
        "introducedLesson": 21
      },
      {
        "id": "noun-attached-sg",
        "title": "Noun + attached pronoun, singular-referent set, on a singular-noun host",
        "columns": [
          "Word",
          "Gender",
          "Suffix",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "noun-attached-sg-3ms",
            "cells": [
              "דָּבָר",
              "Masculine",
              "3ms",
              "דְּבָרוֹ",
              "his word"
            ],
            "introducedLesson": 22,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-sg-3fs",
            "cells": [
              "דָּבָר",
              "Masculine",
              "3fs",
              "דְּבָרָהּ",
              "her word"
            ],
            "introducedLesson": 22,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-sg-2ms",
            "cells": [
              "דָּבָר",
              "Masculine",
              "2ms",
              "דְּבָרְךָ",
              "your (ms) word"
            ],
            "introducedLesson": 22,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-sg-2fs",
            "cells": [
              "דָּבָר",
              "Masculine",
              "2fs",
              "דְּבָרֵךְ",
              "your (fs) word"
            ],
            "introducedLesson": 22,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-sg-1cs",
            "cells": [
              "דָּבָר",
              "Masculine",
              "1cs",
              "דְּבָרִי",
              "my word"
            ],
            "introducedLesson": 22,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-sg-3mp",
            "cells": [
              "דָּבָר",
              "Masculine",
              "3mp",
              "דְּבָרָם",
              "their word"
            ],
            "introducedLesson": 31,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-sg-3fp",
            "cells": [
              "דָּבָר",
              "Masculine",
              "3fp",
              "דְּבָרָן",
              "their word"
            ],
            "introducedLesson": 31,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-sg-2mp",
            "cells": [
              "דָּבָר",
              "Masculine",
              "2mp",
              "דְּבַרְכֶם",
              "your (mp) word"
            ],
            "introducedLesson": 31,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-sg-2fp",
            "cells": [
              "דָּבָר",
              "Masculine",
              "2fp",
              "דְּבַרְכֶן",
              "your (fp) word"
            ],
            "introducedLesson": 31,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-sg-1cp",
            "cells": [
              "דָּבָר",
              "Masculine",
              "1cp",
              "דְּבָרֵנוּ",
              "our word"
            ],
            "introducedLesson": 31,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-sg-3ms-fem-host",
            "cells": [
              "תּוֹרָה",
              "Feminine",
              "3ms",
              "תּוֹרָתוֹ",
              "his teaching/law; illustrates that feminine ־ָה nouns take suffixes on the ־ת (נסמך) stem"
            ],
            "introducedLesson": 22,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 22,
            "page": "68-69",
            "pageCount": 2
          },
          {
            "lesson": 31,
            "page": 91
          }
        ],
        "introducedLesson": 22
      },
      {
        "id": "noun-attached-pl",
        "title": "Noun + attached pronoun, on a plural-noun host (bracketed forms)",
        "columns": [
          "Word",
          "Gender",
          "Suffix",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "noun-attached-pl-3ms",
            "cells": [
              "דָּבָר",
              "Masculine",
              "3ms",
              "דְּבָרָיו",
              "his words"
            ],
            "introducedLesson": 22,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-pl-3fs",
            "cells": [
              "דָּבָר",
              "Masculine",
              "3fs",
              "דְּבָרֶיהָ",
              "her words"
            ],
            "introducedLesson": 31,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-pl-2ms",
            "cells": [
              "דָּבָר",
              "Masculine",
              "2ms",
              "דְּבָרֶיךָ",
              "your (ms) words"
            ],
            "introducedLesson": 31,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-pl-2fs",
            "cells": [
              "דָּבָר",
              "Masculine",
              "2fs",
              "דְּבָרַיִךְ",
              "your (fs) words"
            ],
            "introducedLesson": 31,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-pl-1cs",
            "cells": [
              "דָּבָר",
              "Masculine",
              "1cs",
              "דְּבָרַי",
              "my words"
            ],
            "introducedLesson": 31,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-pl-3mp",
            "cells": [
              "דָּבָר",
              "Masculine",
              "3mp",
              "דִּבְרֵיהֶם",
              "their words"
            ],
            "introducedLesson": 31,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-pl-3fp",
            "cells": [
              "דָּבָר",
              "Masculine",
              "3fp",
              "דִּבְרֵיהֶן",
              "their words"
            ],
            "introducedLesson": 31,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-pl-2mp",
            "cells": [
              "דָּבָר",
              "Masculine",
              "2mp",
              "דִּבְרֵיכֶם",
              "your (mp) words"
            ],
            "introducedLesson": 31,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-pl-2fp",
            "cells": [
              "דָּבָר",
              "Masculine",
              "2fp",
              "דִּבְרֵיכֶן",
              "your (fp) words"
            ],
            "introducedLesson": 31,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-pl-1cp",
            "cells": [
              "דָּבָר",
              "Masculine",
              "1cp",
              "דִּבְרֵינוּ",
              "our words"
            ],
            "introducedLesson": 31,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-attached-pl-3ms-fem-host",
            "cells": [
              "תּוֹרָה",
              "Feminine",
              "3ms",
              "תּוֹרוֹתָיו",
              "his teachings/laws; feminine-host equivalent, only seen printed in the appendix chart"
            ],
            "introducedLesson": 31,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 22,
            "page": 69
          },
          {
            "lesson": 31,
            "page": 91
          },
          {
            "appendix": "B",
            "page": "a-13"
          }
        ],
        "introducedLesson": 22
      },
      {
        "id": "verb-qal-imperfect",
        "title": "Qal Imperfect (שמר)",
        "columns": [
          "Person",
          "Gender",
          "Number",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "verb-qal-imperfect-3ms",
            "cells": [
              "3",
              "Masculine",
              "Singular",
              "יִשְׁמֹר",
              "he will guard; prefix conjugation"
            ],
            "introducedLesson": 23,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-imperfect-2ms-3fs",
            "cells": [
              "2",
              "Masculine",
              "Singular",
              "תִּשְׁמֹר",
              "identical written form for \"you (ms) will guard\" and \"she will guard\""
            ],
            "introducedLesson": 23,
            "appendixOnly": false,
            "ambiguityNote": "Book prints both 2ms and 3fs as תִּשְׁמֹר with no morphological distinction; disambiguated only by subject (explicit pronoun/noun) or discourse context."
          },
          {
            "id": "verb-qal-imperfect-2fs",
            "cells": [
              "2",
              "Feminine",
              "Singular",
              "תִּשְׁמְרִי",
              ""
            ],
            "introducedLesson": 23,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-imperfect-1cs",
            "cells": [
              "1",
              "Common",
              "Singular",
              "אֶשְׁמֹר",
              ""
            ],
            "introducedLesson": 23,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-imperfect-3mp",
            "cells": [
              "3",
              "Masculine",
              "Plural",
              "יִשְׁמְרוּ",
              ""
            ],
            "introducedLesson": 27,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-imperfect-2mp",
            "cells": [
              "2",
              "Masculine",
              "Plural",
              "תִּשְׁמְרוּ",
              ""
            ],
            "introducedLesson": 27,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-imperfect-3fp-2fp",
            "cells": [
              "3",
              "Feminine",
              "Plural",
              "תִּשְׁמֹרְנָה",
              "identical written form for \"they (fp) will guard\" and \"you (fp) will guard\""
            ],
            "introducedLesson": 27,
            "appendixOnly": false,
            "ambiguityNote": "Book prints both 3fp and 2fp as תִּשְׁמֹרְנָה with no morphological distinction; disambiguated only by subject/context."
          },
          {
            "id": "verb-qal-imperfect-1cp",
            "cells": [
              "1",
              "Common",
              "Plural",
              "נִשְׁמֹר",
              ""
            ],
            "introducedLesson": 27,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 23,
            "page": 71
          },
          {
            "lesson": 27,
            "page": 81
          }
        ],
        "introducedLesson": 23
      },
      {
        "id": "verb-qal-infinitive",
        "title": "Qal Infinitive (construct)",
        "columns": [
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "verb-qal-infinitive-base",
            "cells": [
              "שְׁמֹר",
              "not inflected for person/gender/number; may take an attached pronoun as subject or object depending on context"
            ],
            "introducedLesson": 24,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 24,
            "page": 74
          }
        ],
        "introducedLesson": 24
      },
      {
        "id": "verb-qal-adverbial-infinitive",
        "title": "Qal Adverbial Infinitive (absolute)",
        "columns": [
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "verb-qal-adverbial-infinitive-base",
            "cells": [
              "שָׁמוֹר",
              "not inflected; distinct spelling (holem-vav) from the construct infinitive שְׁמֹר; paired with a finite verb of the same root/binyan for modal emphasis"
            ],
            "introducedLesson": 25,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 25,
            "page": 77
          }
        ],
        "introducedLesson": 25
      },
      {
        "id": "verb-haya-imperfect",
        "title": "Qal Imperfect of היה (irregular \"to be\")",
        "columns": [
          "Person",
          "Gender",
          "Number",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "verb-haya-imperfect-3ms",
            "cells": [
              "3",
              "Masculine",
              "Singular",
              "יִהְיֶה",
              "he will be"
            ],
            "introducedLesson": 28,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-haya-imperfect-2ms-3fs",
            "cells": [
              "2",
              "Masculine",
              "Singular",
              "תִּהְיֶה",
              "same 2ms/3fs syncretism as regular Qal Imperfect"
            ],
            "introducedLesson": 28,
            "appendixOnly": false,
            "ambiguityNote": "Book prints both 2ms and 3fs as תִּהְיֶה; disambiguated only by subject/context."
          },
          {
            "id": "verb-haya-imperfect-2fs",
            "cells": [
              "2",
              "Feminine",
              "Singular",
              "תִּהְיִי",
              ""
            ],
            "introducedLesson": 28,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-haya-imperfect-1cs",
            "cells": [
              "1",
              "Common",
              "Singular",
              "אֶהְיֶה",
              ""
            ],
            "introducedLesson": 28,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-haya-imperfect-3mp",
            "cells": [
              "3",
              "Masculine",
              "Plural",
              "יִהְיוּ",
              ""
            ],
            "introducedLesson": 28,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-haya-imperfect-2mp",
            "cells": [
              "2",
              "Masculine",
              "Plural",
              "תִּהְיוּ",
              ""
            ],
            "introducedLesson": 28,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-haya-imperfect-3fp-2fp",
            "cells": [
              "3",
              "Feminine",
              "Plural",
              "תִּהְיֶינָה",
              "same 3fp/2fp syncretism as regular Qal Imperfect"
            ],
            "introducedLesson": 28,
            "appendixOnly": false,
            "ambiguityNote": "Book prints both 3fp and 2fp as תִּהְיֶינָה; disambiguated only by subject/context."
          },
          {
            "id": "verb-haya-imperfect-1cp",
            "cells": [
              "1",
              "Common",
              "Plural",
              "נִהְיֶה",
              ""
            ],
            "introducedLesson": 28,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 28,
            "page": 83
          }
        ],
        "introducedLesson": 28
      },
      {
        "id": "verb-piel-recognition",
        "title": "Piel recognition forms (Perfect/Imperfect 3ms)",
        "columns": [
          "Word",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "verb-piel-recognition-perf-3ms-qbts",
            "cells": [
              "קִבֵּץ",
              "קִבֵּץ",
              "gather; dagesh in R2 marks Piel"
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-piel-recognition-impf-3ms-qbts",
            "cells": [
              "קִבֵּץ",
              "יְקַבֵּץ",
              "he will gather"
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-piel-recognition-perf-3ms-kbd",
            "cells": [
              "כִּבֵּד",
              "כִּבֵּד",
              "he considered [someone] honored; cf. Qal כָּבֵד 'he was heavy'"
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-piel-recognition-perf-3ms-dbr",
            "cells": [
              "דִּבֶּר",
              "דִּבֶּר",
              "he spoke; root not used in Qal"
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-piel-recognition-perf-3ms-tswh",
            "cells": [
              "צִוָּה",
              "צִוָּה",
              "he commanded; root not used in Qal"
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-piel-recognition-perf-3ms-brk",
            "cells": [
              "בֵּרַךְ",
              "בֵּרַךְ",
              "bless; ר blocks the dagesh so no visible R2 doubling Textbook prints alternate spellings בֵּרֵךְ/בֵּרַךְ (tsere/patach)."
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-piel-recognition-perf-3ms-klh",
            "cells": [
              "כִּלָּה",
              "כִּלָּה",
              "complete, finish (III-He root)"
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 29,
            "page": "85-86",
            "pageCount": 2
          }
        ],
        "introducedLesson": 29
      },
      {
        "id": "verb-hifil-recognition",
        "title": "Hifil recognition forms (Perfect/Imperfect/Imperative)",
        "columns": [
          "Word",
          "Person",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "verb-hifil-recognition-perf-3ms-shlk",
            "cells": [
              "הִשְׁלִיךְ",
              "3",
              "הִשְׁלִיךְ",
              "he threw X; root not used in Qal or Piel"
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-hifil-recognition-impf-3ms-shlk",
            "cells": [
              "הִשְׁלִיךְ",
              "3",
              "יַשְׁלִיךְ",
              "he will throw X"
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-hifil-recognition-impv-2ms-shlk",
            "cells": [
              "הִשְׁלִיךְ",
              "2",
              "הַשְׁלֵךְ",
              "throw X! (footnote points to appendix C for a full Piel/Hifil paradigm)"
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-hifil-recognition-perf-3ms-mlk",
            "cells": [
              "הִמְלִיךְ",
              "3",
              "הִמְלִיךְ",
              "he made (someone) king; cf. Qal מָלַךְ 'he was king'"
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-hifil-recognition-perf-3ms-kbd",
            "cells": [
              "הִכְבִּיד",
              "3",
              "הִכְבִּיד",
              "he made [someone] honored/heavy"
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-hifil-recognition-perf-3ms-shkm",
            "cells": [
              "הִשְׁכִּים",
              "3",
              "הִשְׁכִּים",
              "he rose early [in the morning]; root not used in Qal or Piel"
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-hifil-recognition-perf-3ms-tzlch",
            "cells": [
              "הִצְלִיחַ",
              "3",
              "הִצְלִיחַ",
              "make successful, show experience"
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-hifil-recognition-perf-3ms-shb",
            "cells": [
              "הִשְׁבִּיעַ",
              "3",
              "הִשְׁבִּיעַ",
              "cause to swear an oath"
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-hifil-recognition-perf-3ms-shcht",
            "cells": [
              "הִשְׁחִית",
              "3",
              "הִשְׁחִית",
              "spoil, destroy (III-Weak root)"
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-hifil-recognition-perf-3ms-bdl",
            "cells": [
              "הִבְדִּיל",
              "3",
              "הִבְדִּיל",
              "divide, separate (new vocab in L42, reinforces Hifil recognition)"
            ],
            "introducedLesson": 42,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 29,
            "page": "85-86",
            "pageCount": 2
          },
          {
            "lesson": 42,
            "page": 117
          }
        ],
        "introducedLesson": 29
      },
      {
        "id": "numeral-cardinal-1-10",
        "title": "Cardinal numbers 1-10",
        "columns": [
          "Word",
          "Gender",
          "Number",
          "State",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "numeral-cardinal-1-10-1-f",
            "cells": [
              "אַחַת",
              "Feminine",
              "Singular",
              "Absolute",
              "אַחַת",
              "one (f); modifies feminine nouns as an adjective, agrees in gender"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": "Absolute and construct spelled identically."
          },
          {
            "id": "numeral-cardinal-1-10-1-m",
            "cells": [
              "אֶחָד",
              "Masculine",
              "Singular",
              "Absolute",
              "אֶחָד",
              "one (m)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-1-m-cst",
            "cells": [
              "אֶחָד",
              "Masculine",
              "Singular",
              "Construct",
              "אַחַד",
              "Construct (bound) form of one, masculine."
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-2-f-abs",
            "cells": [
              "שְׁתַּיִם",
              "Feminine",
              "Dual",
              "Absolute",
              "שְׁתַּיִם",
              "two (f) absolute"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-2-f-cstr",
            "cells": [
              "שְׁתַּיִם",
              "Feminine",
              "Dual",
              "Construct",
              "שְׁתֵּי",
              "two (f) construct"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-2-m-abs",
            "cells": [
              "שְׁנַיִם",
              "Masculine",
              "Dual",
              "Absolute",
              "שְׁנַיִם",
              "two (m) absolute"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-2-m-cstr",
            "cells": [
              "שְׁנַיִם",
              "Masculine",
              "Dual",
              "Construct",
              "שְׁנֵי",
              "two (m) construct; 2-10 are nouns usable in apposition or construct with the counted noun"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-3-f-abs",
            "cells": [
              "שָׁלֹשׁ",
              "Feminine",
              "Singular",
              "Absolute",
              "שָׁלֹשׁ",
              "three (f) absolute; 3-10 take the opposite gender form from the noun they modify"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-3-f-cstr",
            "cells": [
              "שָׁלֹשׁ",
              "Feminine",
              "Singular",
              "Construct",
              "שְׁלֹשׁ",
              "three (f) construct"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-3-m-abs",
            "cells": [
              "שְׁלֹשָׁה",
              "Masculine",
              "Singular",
              "Absolute",
              "שְׁלֹשָׁה",
              "three (m) absolute"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-3-m-cstr",
            "cells": [
              "שְׁלֹשָׁה",
              "Masculine",
              "Singular",
              "Construct",
              "שְׁלֹשֶׁת",
              "three (m) construct"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-4-f",
            "cells": [
              "אַרְבַּע",
              "Feminine",
              "Singular",
              "Absolute",
              "אַרְבַּע",
              "four (f)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": "Absolute and construct spelled identically."
          },
          {
            "id": "numeral-cardinal-1-10-4-m-abs",
            "cells": [
              "אַרְבָּעָה",
              "Masculine",
              "Singular",
              "Absolute",
              "אַרְבָּעָה",
              "four (m) absolute"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-4-m-cstr",
            "cells": [
              "אַרְבָּעָה",
              "Masculine",
              "Singular",
              "Construct",
              "אַרְבַּעַת",
              "four (m) construct"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-5-f-abs",
            "cells": [
              "חָמֵשׁ",
              "Feminine",
              "Singular",
              "Absolute",
              "חָמֵשׁ",
              "five (f) absolute"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-5-f-cstr",
            "cells": [
              "חָמֵשׁ",
              "Feminine",
              "Singular",
              "Construct",
              "חֲמֵשׁ",
              "five (f) construct"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-5-m-abs",
            "cells": [
              "חֲמִשָּׁה",
              "Masculine",
              "Singular",
              "Absolute",
              "חֲמִשָּׁה",
              "five (m) absolute"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-5-m-cstr",
            "cells": [
              "חֲמִשָּׁה",
              "Masculine",
              "Singular",
              "Construct",
              "חֲמֵשֶׁת",
              "five (m) construct"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-6-f",
            "cells": [
              "שֵׁשׁ",
              "Feminine",
              "Singular",
              "Absolute",
              "שֵׁשׁ",
              "six (f)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": "Absolute and construct spelled identically."
          },
          {
            "id": "numeral-cardinal-1-10-6-m-abs",
            "cells": [
              "שִׁשָּׁה",
              "Masculine",
              "Singular",
              "Absolute",
              "שִׁשָּׁה",
              "six (m) absolute"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-6-m-cstr",
            "cells": [
              "שִׁשָּׁה",
              "Masculine",
              "Singular",
              "Construct",
              "שֵׁשֶׁת",
              "six (m) construct"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-7-f-abs",
            "cells": [
              "שֶׁבַע",
              "Feminine",
              "Singular",
              "Absolute",
              "שֶׁבַע",
              "seven (f) absolute"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-7-f-cstr",
            "cells": [
              "שֶׁבַע",
              "Feminine",
              "Singular",
              "Construct",
              "שְׁבַע",
              "seven (f) construct"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-7-m-abs",
            "cells": [
              "שִׁבְעָה",
              "Masculine",
              "Singular",
              "Absolute",
              "שִׁבְעָה",
              "seven (m) absolute"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-7-m-cstr",
            "cells": [
              "שִׁבְעָה",
              "Masculine",
              "Singular",
              "Construct",
              "שִׁבְעַת",
              "seven (m) construct"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-8-f",
            "cells": [
              "שְׁמֹנֶה",
              "Feminine",
              "Singular",
              "Absolute",
              "שְׁמֹנֶה",
              "eight (f)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": "Absolute and construct spelled identically."
          },
          {
            "id": "numeral-cardinal-1-10-8-m-abs",
            "cells": [
              "שְׁמֹנָה",
              "Masculine",
              "Singular",
              "Absolute",
              "שְׁמֹנָה",
              "eight (m) absolute"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-8-m-cstr",
            "cells": [
              "שְׁמֹנָה",
              "Masculine",
              "Singular",
              "Construct",
              "שְׁמֹנַת",
              "eight (m) construct"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-9-f-abs",
            "cells": [
              "תֵּשַׁע",
              "Feminine",
              "Singular",
              "Absolute",
              "תֵּשַׁע",
              "nine (f) absolute"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-9-f-cstr",
            "cells": [
              "תֵּשַׁע",
              "Feminine",
              "Singular",
              "Construct",
              "תְּשַׁע",
              "nine (f) construct"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-9-m-abs",
            "cells": [
              "תִּשְׁעָה",
              "Masculine",
              "Singular",
              "Absolute",
              "תִּשְׁעָה",
              "nine (m) absolute"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-9-m-cstr",
            "cells": [
              "תִּשְׁעָה",
              "Masculine",
              "Singular",
              "Construct",
              "תִּשְׁעַת",
              "nine (m) construct"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-10-f",
            "cells": [
              "עֶשֶׂר",
              "Feminine",
              "Singular",
              "Absolute",
              "עֶשֶׂר",
              "ten (f); first printed as vocab in L29, formalized in the L45 numerals table"
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": "Absolute and construct spelled identically."
          },
          {
            "id": "numeral-cardinal-1-10-10-m-abs",
            "cells": [
              "עֲשָׂרָה",
              "Masculine",
              "Singular",
              "Absolute",
              "עֲשָׂרָה",
              "ten (m) absolute; first printed as vocab in L29"
            ],
            "introducedLesson": 29,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-cardinal-1-10-10-m-cstr",
            "cells": [
              "עֲשָׂרָה",
              "Masculine",
              "Singular",
              "Construct",
              "עֲשֶׂרֶת",
              "ten (m) construct"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 29,
            "page": 86
          },
          {
            "lesson": 45,
            "page": 125
          }
        ],
        "introducedLesson": 29
      },
      {
        "id": "pron-demonstrative",
        "title": "Demonstrative pronouns: near/far x gender x number",
        "columns": [
          "Gender",
          "Number",
          "Deixis",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "pron-demonstrative-near-ms",
            "cells": [
              "Masculine",
              "Singular",
              "Near",
              "זֶה",
              "this"
            ],
            "introducedLesson": 33,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "pron-demonstrative-near-fs",
            "cells": [
              "Feminine",
              "Singular",
              "Near",
              "זֹאת",
              "this"
            ],
            "introducedLesson": 33,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "pron-demonstrative-near-cp",
            "cells": [
              "Common",
              "Plural",
              "Near",
              "אֵלֶּה",
              "these; one common-gender plural form"
            ],
            "introducedLesson": 33,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "pron-demonstrative-far-ms",
            "cells": [
              "Masculine",
              "Singular",
              "Far",
              "הוּא",
              "that; identical to the 3ms independent pronoun (pron-subject-3ms)"
            ],
            "introducedLesson": 33,
            "appendixOnly": false,
            "ambiguityNote": "same spelling as the 3ms subject pronoun; context (predicate position vs. modifying/substituting a noun) distinguishes the demonstrative use"
          },
          {
            "id": "pron-demonstrative-far-fs",
            "cells": [
              "Feminine",
              "Singular",
              "Far",
              "הִיא",
              "that; identical to the 3fs independent pronoun"
            ],
            "introducedLesson": 33,
            "appendixOnly": false,
            "ambiguityNote": "same spelling as the 3fs subject pronoun; context distinguishes the demonstrative use"
          },
          {
            "id": "pron-demonstrative-far-mp",
            "cells": [
              "Masculine",
              "Plural",
              "Far",
              "הֵמָּה",
              "those"
            ],
            "introducedLesson": 33,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "pron-demonstrative-far-fp",
            "cells": [
              "Feminine",
              "Plural",
              "Far",
              "הֵנָּה",
              "those"
            ],
            "introducedLesson": 33,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 33,
            "page": 96
          }
        ],
        "introducedLesson": 33
      },
      {
        "id": "noun-segolate",
        "title": "Segolate noun pattern: first-syllable stress in the singular",
        "columns": [
          "Word",
          "Gender",
          "Number",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "noun-segolate-melekh-sg",
            "cells": [
              "מֶלֶךְ",
              "Masculine",
              "Singular",
              "מֶלֶךְ",
              "king"
            ],
            "introducedLesson": 34,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-segolate-boker-sg",
            "cells": [
              "בֹּקֶר",
              "Masculine",
              "Singular",
              "בֹּקֶר",
              "morning"
            ],
            "introducedLesson": 34,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-segolate-naar-sg",
            "cells": [
              "נַעַר",
              "Masculine",
              "Singular",
              "נַעַר",
              "young man"
            ],
            "introducedLesson": 34,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-segolate-erets-sg",
            "cells": [
              "אֶרֶץ",
              "Feminine",
              "Singular",
              "אֶרֶץ",
              "land"
            ],
            "introducedLesson": 34,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-segolate-eved-sg",
            "cells": [
              "עֶבֶד",
              "Masculine",
              "Singular",
              "עֶבֶד",
              "servant"
            ],
            "introducedLesson": 34,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-segolate-nefesh-sg",
            "cells": [
              "נֶפֶשׁ",
              "Feminine",
              "Singular",
              "נֶפֶשׁ",
              "life, self"
            ],
            "introducedLesson": 34,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-segolate-malka-sg",
            "cells": [
              "מַלְכָּה",
              "Feminine",
              "Singular",
              "מַלְכָּה",
              "queen; fem. ־ָה ending shifts stress to the final syllable, unlike the plain segolate pattern"
            ],
            "introducedLesson": 34,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-segolate-naara-sg",
            "cells": [
              "נַעֲרָה",
              "Feminine",
              "Singular",
              "נַעֲרָה",
              "young woman; same final-stress note as מַלְכָּה"
            ],
            "introducedLesson": 34,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-segolate-melekh-pl",
            "cells": [
              "מֶלֶךְ",
              "Masculine",
              "Plural",
              "מְלָכִים",
              "kings"
            ],
            "introducedLesson": 34,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-segolate-malka-pl",
            "cells": [
              "מַלְכָּה",
              "Feminine",
              "Plural",
              "מְלָכוֹת",
              "queens"
            ],
            "introducedLesson": 34,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-segolate-naar-pl",
            "cells": [
              "נַעַר",
              "Masculine",
              "Plural",
              "נְעָרִים",
              "young men"
            ],
            "introducedLesson": 34,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-segolate-naara-pl",
            "cells": [
              "נַעֲרָה",
              "Feminine",
              "Plural",
              "נְעָרוֹת",
              "young women"
            ],
            "introducedLesson": 34,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-segolate-eved-pl",
            "cells": [
              "עֶבֶד",
              "Masculine",
              "Plural",
              "עֲבָדִים",
              "servants"
            ],
            "introducedLesson": 34,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-segolate-boker-pl",
            "cells": [
              "בֹּקֶר",
              "Masculine",
              "Plural",
              "בְּקָרִים",
              "mornings"
            ],
            "introducedLesson": 34,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-segolate-erets-pl",
            "cells": [
              "אֶרֶץ",
              "Feminine",
              "Plural",
              "אֲרָצוֹת",
              "lands"
            ],
            "introducedLesson": 34,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "noun-segolate-nefesh-pl",
            "cells": [
              "נֶפֶשׁ",
              "Feminine",
              "Plural",
              "נְפָשׁוֹת",
              "lives, souls"
            ],
            "introducedLesson": 34,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 34,
            "page": 98
          }
        ],
        "introducedLesson": 34
      },
      {
        "id": "verb-past-narrative",
        "title": "Past Narrative (Qal, שמר/היה)",
        "columns": [
          "Word",
          "Person",
          "Gender",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "verb-past-narrative-3ms",
            "cells": [
              "שָׁמַר",
              "3",
              "Masculine",
              "וַיִּשְׁמֹר",
              "he guarded; vav-conjunction + dagesh forte distinguishes from Imperfect יִשְׁמֹר; triggers Verb-Subject word order"
            ],
            "introducedLesson": 35,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-past-narrative-1cs",
            "cells": [
              "שָׁמַר",
              "1",
              "Common",
              "וָאֶשְׁמֹר",
              "I guarded; dagesh is regularly absent in 1cs Past Narrative forms"
            ],
            "introducedLesson": 35,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-past-narrative-3ms-haya",
            "cells": [
              "הָיָה",
              "3",
              "Masculine",
              "וַיְהִי",
              "extremely frequent; means \"was/became/came\" or simply marks the start of a narrative episode (often left untranslated as \"and it was\")"
            ],
            "introducedLesson": 36,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 35,
            "page": 100
          },
          {
            "lesson": 36,
            "page": 102
          }
        ],
        "introducedLesson": 35
      },
      {
        "id": "verb-nifal-recognition",
        "title": "Nifal recognition forms (Perfect/Imperfect/Imperative)",
        "columns": [
          "Word",
          "Person",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "verb-nifal-recognition-perf-3ms-shmr",
            "cells": [
              "נִשְׁמַר",
              "3",
              "נִשְׁמַר",
              "he/it was guarded; נ prefix marks Nifal Perfect"
            ],
            "introducedLesson": 37,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-nifal-recognition-impf-3ms-shmr",
            "cells": [
              "נִשְׁמַר",
              "3",
              "יִשָּׁמֵר",
              "he/it will be guarded; dagesh in R1 + hireq under prefix"
            ],
            "introducedLesson": 37,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-nifal-recognition-impv-2ms-shmr",
            "cells": [
              "נִשְׁמַר",
              "2",
              "הִשָּׁמֵר",
              "be guarded! (reflexive sense)"
            ],
            "introducedLesson": 37,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-nifal-recognition-perf-3ms-rah",
            "cells": [
              "נִרְאָה",
              "3",
              "נִרְאָה",
              "show oneself, appear (new vocab in L42)"
            ],
            "introducedLesson": 42,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 37,
            "page": 104
          },
          {
            "lesson": 42,
            "page": 117
          }
        ],
        "introducedLesson": 37
      },
      {
        "id": "verb-hitpael-recognition",
        "title": "Hitpael recognition forms (Perfect/Imperfect/Imperative)",
        "columns": [
          "Person",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "verb-hitpael-recognition-perf-3ms-qbts",
            "cells": [
              "3",
              "הִתְקַבֵּץ",
              "he gathered together [with others]; הִת prefix, dagesh in R2"
            ],
            "introducedLesson": 37,
            "appendixOnly": false,
            "ambiguityNote": "Spelled identically to the 2ms Imperative הִתְקַבֵּץ; context (subject vs. command) distinguishes."
          },
          {
            "id": "verb-hitpael-recognition-impv-2ms-qbts",
            "cells": [
              "2",
              "הִתְקַבֵּץ",
              "gather together [with others]!"
            ],
            "introducedLesson": 37,
            "appendixOnly": false,
            "ambiguityNote": "Spelled identically to the 3ms Perfect; context distinguishes."
          },
          {
            "id": "verb-hitpael-recognition-impf-3ms-qbts",
            "cells": [
              "3",
              "יִתְקַבֵּץ",
              "he will gather together [with others]; ת follows inflectional prefix"
            ],
            "introducedLesson": 37,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 37,
            "page": 104
          }
        ],
        "introducedLesson": 37
      },
      {
        "id": "verb-qal-imperative",
        "title": "Qal Imperative (guard, שמר)",
        "columns": [
          "Gender",
          "Number",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "verb-qal-imperative-2ms",
            "cells": [
              "Masculine",
              "Singular",
              "שְׁמֹר",
              "guard! (ms)"
            ],
            "introducedLesson": 39,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-imperative-2fs",
            "cells": [
              "Feminine",
              "Singular",
              "שִׁמְרִי",
              "guard! (fs)"
            ],
            "introducedLesson": 39,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-imperative-2mp",
            "cells": [
              "Masculine",
              "Plural",
              "שִׁמְרוּ",
              "guard! (mp)"
            ],
            "introducedLesson": 39,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-imperative-2fp",
            "cells": [
              "Feminine",
              "Plural",
              "שְׁמֹרְנָה",
              "guard! (fp)"
            ],
            "introducedLesson": 39,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 39,
            "page": 110
          }
        ],
        "introducedLesson": 39
      },
      {
        "id": "verb-qal-jussive",
        "title": "Qal Jussive / Cohortative",
        "columns": [
          "Word",
          "Person",
          "Gender",
          "Number",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "verb-qal-jussive-coh-1cs-shmr",
            "cells": [
              "שָׁמַר",
              "1",
              "Common",
              "Singular",
              "אֶשְׁמְרָה",
              "let me/I shall guard (Cohortative); 1st-person Jussives add ה- in all binyanim"
            ],
            "introducedLesson": 39,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-jussive-coh-1cp-shmr",
            "cells": [
              "שָׁמַר",
              "1",
              "Common",
              "Plural",
              "נִשְׁמְרָה",
              "let us/we shall guard (Cohortative)"
            ],
            "introducedLesson": 39,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-jussive-3ms-qum",
            "cells": [
              "קוּם",
              "3",
              "Masculine",
              "Singular",
              "יָקֹם",
              "let him stand up; hollow root shortens the theme vowel in the Jussive"
            ],
            "introducedLesson": 39,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-impf-3ms-qum",
            "cells": [
              "קוּם",
              "3",
              "Masculine",
              "Singular",
              "יָקוּם",
              "he will stand up (Imperfect, printed as contrast with the Jussive)"
            ],
            "introducedLesson": 39,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 39,
            "page": 109
          }
        ],
        "introducedLesson": 39
      },
      {
        "id": "verb-qal-participle",
        "title": "Qal Active and Passive Participle",
        "columns": [
          "Word",
          "Gender",
          "Number",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "verb-qal-participle-act-ms-abs",
            "cells": [
              "אָמַר",
              "Masculine",
              "Singular",
              "אֹמֵר",
              "active participle 'saying'; absolute state (table also gives a construct column, not encodable as a verb feature here)"
            ],
            "introducedLesson": 42,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-participle-act-ms-cstr",
            "cells": [
              "אָמַר",
              "Masculine",
              "Singular",
              "אֹמַר",
              "active participle, construct-state form (נִסְמָךְ column)"
            ],
            "introducedLesson": 42,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-participle-act-fs-abs",
            "cells": [
              "אָמַר",
              "Feminine",
              "Singular",
              "אֹמֶרֶת / אֹמְרָה",
              "two legitimate fs absolute participle forms"
            ],
            "introducedLesson": 42,
            "appendixOnly": false,
            "ambiguityNote": "Both forms printed side by side as equally valid fs participle spellings."
          },
          {
            "id": "verb-qal-participle-act-fs-cstr",
            "cells": [
              "אָמַר",
              "Feminine",
              "Singular",
              "אֹמֶרֶת / אֹמְרַת",
              "two fs construct-state participle forms"
            ],
            "introducedLesson": 42,
            "appendixOnly": false,
            "ambiguityNote": "Both forms printed side by side."
          },
          {
            "id": "verb-qal-participle-act-mp-abs",
            "cells": [
              "אָמַר",
              "Masculine",
              "Plural",
              "אֹמְרִים",
              "mp absolute"
            ],
            "introducedLesson": 42,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-participle-act-mp-cstr",
            "cells": [
              "אָמַר",
              "Masculine",
              "Plural",
              "אֹמְרֵי",
              "mp construct"
            ],
            "introducedLesson": 42,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-participle-act-fp",
            "cells": [
              "אָמַר",
              "Feminine",
              "Plural",
              "אֹמְרוֹת",
              "fp; absolute and construct are spelled the same"
            ],
            "introducedLesson": 42,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-participle-pass-ms-abscstr",
            "cells": [
              "אָרוּר",
              "Masculine",
              "Singular",
              "אָרוּר",
              "passive participle 'cursed'; absolute and construct spelled the same for ms"
            ],
            "introducedLesson": 42,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-participle-pass-fs-abs",
            "cells": [
              "אָרוּר",
              "Feminine",
              "Singular",
              "אֲרוּרָה",
              "fs absolute passive participle"
            ],
            "introducedLesson": 42,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-participle-pass-fs-cstr",
            "cells": [
              "אָרוּר",
              "Feminine",
              "Singular",
              "אֲרוּרַת",
              "fs construct passive participle"
            ],
            "introducedLesson": 42,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-participle-pass-mp-abs",
            "cells": [
              "אָרוּר",
              "Masculine",
              "Plural",
              "אֲרוּרִים",
              "mp absolute"
            ],
            "introducedLesson": 42,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-participle-pass-mp-cstr",
            "cells": [
              "אָרוּר",
              "Masculine",
              "Plural",
              "אֲרוּרֵי",
              "mp construct"
            ],
            "introducedLesson": 42,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "verb-qal-participle-pass-fp",
            "cells": [
              "אָרוּר",
              "Feminine",
              "Plural",
              "אֲרוּרוֹת",
              "fp; absolute and construct spelled the same"
            ],
            "introducedLesson": 42,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 42,
            "page": "116-117",
            "pageCount": 2
          }
        ],
        "introducedLesson": 42
      },
      {
        "id": "numeral-ordinal-1-10",
        "title": "Ordinal numbers 1st-10th",
        "columns": [
          "Word",
          "Gender",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "numeral-ordinal-1-10-1-m",
            "cells": [
              "רִאשׁוֹן",
              "Masculine",
              "רִאשׁוֹן",
              "1st (m)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-1-f",
            "cells": [
              "רִאשׁוֹן",
              "Feminine",
              "רִאשׁוֹנָה",
              "1st (f)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-2-m",
            "cells": [
              "שֵׁנִי",
              "Masculine",
              "שֵׁנִי",
              "2nd (m)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-2-f",
            "cells": [
              "שֵׁנִי",
              "Feminine",
              "שֵׁנִית",
              "2nd (f)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-3-m",
            "cells": [
              "שְׁלִישִׁי",
              "Masculine",
              "שְׁלִישִׁי",
              "3rd (m)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-3-f",
            "cells": [
              "שְׁלִישִׁי",
              "Feminine",
              "שְׁלִישִׁית",
              "3rd (f)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-4-m",
            "cells": [
              "רְבִיעִי",
              "Masculine",
              "רְבִיעִי",
              "4th (m)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-4-f",
            "cells": [
              "רְבִיעִי",
              "Feminine",
              "רְבִיעִית",
              "4th (f)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-5-m",
            "cells": [
              "חֲמִישִׁי",
              "Masculine",
              "חֲמִישִׁי",
              "5th (m)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-5-f",
            "cells": [
              "חֲמִישִׁי",
              "Feminine",
              "חֲמִישִׁית",
              "5th (f)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-6-m",
            "cells": [
              "שִׁשִּׁי",
              "Masculine",
              "שִׁשִּׁי",
              "6th (m)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-6-f",
            "cells": [
              "שִׁשִּׁי",
              "Feminine",
              "שִׁשִּׁית",
              "6th (f)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-7-m",
            "cells": [
              "שְׁבִיעִי",
              "Masculine",
              "שְׁבִיעִי",
              "7th (m)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-7-f",
            "cells": [
              "שְׁבִיעִי",
              "Feminine",
              "שְׁבִיעִית",
              "7th (f)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-8-m",
            "cells": [
              "שְׁמִינִי",
              "Masculine",
              "שְׁמִינִי",
              "8th (m)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-8-f",
            "cells": [
              "שְׁמִינִי",
              "Feminine",
              "שְׁמִינִית",
              "8th (f)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-9-m",
            "cells": [
              "תְּשִׁיעִי",
              "Masculine",
              "תְּשִׁיעִי",
              "9th (m)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-9-f",
            "cells": [
              "תְּשִׁיעִי",
              "Feminine",
              "תְּשִׁיעִית",
              "9th (f)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-10-m",
            "cells": [
              "עֲשִׂירִי",
              "Masculine",
              "עֲשִׂירִי",
              "10th (m)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-ordinal-1-10-10-f",
            "cells": [
              "עֲשִׂירִי",
              "Feminine",
              "עֲשִׂירִית",
              "10th (f)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 45,
            "page": 125
          }
        ],
        "introducedLesson": 45
      },
      {
        "id": "numeral-11-19",
        "title": "Teens (11-19) and their formation",
        "columns": [
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "numeral-11-19-11-a",
            "cells": [
              "אַחַד עָשָׂר",
              "eleven, form 1 of 2 alternates (units digit + עָשָׂר/עֶשְׂרֵה, agreeing in gender with the noun)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": "Book gives two alternate forms for 11 and 12."
          },
          {
            "id": "numeral-11-19-11-b",
            "cells": [
              "עַשְׁתֵּי עָשָׂר",
              "eleven, alternate form 2"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": "Book gives two alternate forms for 11 and 12."
          },
          {
            "id": "numeral-11-19-12-a",
            "cells": [
              "שְׁנֵים עָשָׂר",
              "twelve, form 1 (matches the standalone 'twelve' vocab entry שְׁנֵים עָשָׂר on p.126)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": "Book gives two alternate forms for 11 and 12."
          },
          {
            "id": "numeral-11-19-12-b",
            "cells": [
              "שְׁנֵי עָשָׂר",
              "twelve, alternate form 2"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": "Book gives two alternate forms for 11 and 12."
          }
        ],
        "sourceRef": [
          {
            "lesson": 45,
            "page": 126
          }
        ],
        "introducedLesson": 45
      },
      {
        "id": "numeral-tens",
        "title": "Tens (multiples of 10, printed examples)",
        "columns": [
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "numeral-tens-20",
            "cells": [
              "עֶשְׂרִים",
              "twenty; tens are formed as the plural of the corresponding units digit (2=20)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-tens-31",
            "cells": [
              "שְׁלוֹשִׁים וְאֶחָד",
              "thirty-one; single integers conjoined to the tens with וְ"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-tens-45",
            "cells": [
              "אַרְבָּעִים וְחָמֵשׁ",
              "forty-five"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 45,
            "page": 126
          }
        ],
        "introducedLesson": 45
      },
      {
        "id": "numeral-hundreds",
        "title": "Hundreds (100/200/300 and the base forms of 100)",
        "columns": [
          "Word",
          "Number",
          "State",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "numeral-hundreds-100-fs-abs",
            "cells": [
              "מֵאָה",
              "Singular",
              "Absolute",
              "מֵאָה",
              "hundred, fs absolute base form"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-hundreds-100-fs-cstr",
            "cells": [
              "מֵאָה",
              "Singular",
              "Construct",
              "מְאַת",
              "hundred, fs construct base form (book labels this 'FS NIS' = construct)"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-hundreds-100-p",
            "cells": [
              "מֵאָה",
              "Plural",
              "",
              "מֵאוֹת",
              "hundreds, plural base form used in 300+"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-hundreds-200",
            "cells": [
              "מָאתַיִם",
              "Dual",
              "",
              "מָאתַיִם",
              "two hundred, dual form of מֵאָה"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          },
          {
            "id": "numeral-hundreds-300",
            "cells": [
              "שְׁלֹשׁ־מֵאוֹת",
              "Plural",
              "",
              "שְׁלֹשׁ־מֵאוֹת",
              "three hundred = cardinal 3 (f) + plural מֵאוֹת, 'and so forth' for higher hundreds"
            ],
            "introducedLesson": 45,
            "appendixOnly": false,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "lesson": 45,
            "page": 126
          }
        ],
        "introducedLesson": 45
      },
      {
        "id": "particle-prep-attached-el",
        "title": "Preposition אֶל ('to/toward') with attached pronouns",
        "columns": [
          "Suffix",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "particle-prep-attached-el-sfx3ms",
            "cells": [
              "3ms",
              "אֵלָיו",
              "preposition 'to/toward' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-el-sfx3fs",
            "cells": [
              "3fs",
              "אֵלֶיהָ",
              "preposition 'to/toward' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-el-sfx2ms",
            "cells": [
              "2ms",
              "אֵלֶיךָ",
              "preposition 'to/toward' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-el-sfx2fs",
            "cells": [
              "2fs",
              "אֵלַיִךְ",
              "preposition 'to/toward' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-el-sfx1cs",
            "cells": [
              "1cs",
              "אֵלַי",
              "preposition 'to/toward' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-el-sfx3mp",
            "cells": [
              "3mp",
              "אֲלֵיהֶם",
              "preposition 'to/toward' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-el-sfx3fp",
            "cells": [
              "3fp",
              "אֲלֵיהֶן",
              "preposition 'to/toward' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-el-sfx2mp",
            "cells": [
              "2mp",
              "אֲלֵיכֶם",
              "preposition 'to/toward' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-el-sfx2fp",
            "cells": [
              "2fp",
              "אֲלֵיכֶן",
              "preposition 'to/toward' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-el-sfx1cp",
            "cells": [
              "1cp",
              "אֵלֵינוּ",
              "preposition 'to/toward' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "appendix": "B",
            "page": "a-14"
          }
        ],
        "appendixOnly": true
      },
      {
        "id": "particle-prep-attached-al",
        "title": "Preposition עַל ('upon/on') with attached pronouns",
        "columns": [
          "Suffix",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "particle-prep-attached-al-sfx3ms",
            "cells": [
              "3ms",
              "עָלָיו",
              "preposition 'upon/on' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-al-sfx3fs",
            "cells": [
              "3fs",
              "עָלֶיהָ",
              "preposition 'upon/on' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-al-sfx2ms",
            "cells": [
              "2ms",
              "עָלֶיךָ",
              "preposition 'upon/on' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-al-sfx2fs",
            "cells": [
              "2fs",
              "עָלַיִךְ",
              "preposition 'upon/on' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-al-sfx1cs",
            "cells": [
              "1cs",
              "עָלַי",
              "preposition 'upon/on' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-al-sfx3mp",
            "cells": [
              "3mp",
              "עֲלֵיהֶם",
              "preposition 'upon/on' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-al-sfx3fp",
            "cells": [
              "3fp",
              "עֲלֵיהֶן",
              "preposition 'upon/on' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-al-sfx2mp",
            "cells": [
              "2mp",
              "עֲלֵיכֶם",
              "preposition 'upon/on' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-al-sfx2fp",
            "cells": [
              "2fp",
              "עֲלֵיכֶן",
              "preposition 'upon/on' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-al-sfx1cp",
            "cells": [
              "1cp",
              "עָלֵינוּ",
              "preposition 'upon/on' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "appendix": "B",
            "page": "a-14"
          }
        ],
        "appendixOnly": true
      },
      {
        "id": "particle-prep-attached-ad",
        "title": "Preposition עַד ('until/as far as') with attached pronouns",
        "columns": [
          "Suffix",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "particle-prep-attached-ad-sfx3ms",
            "cells": [
              "3ms",
              "עָדָיו",
              "preposition 'until/as far as' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-ad-sfx3fs",
            "cells": [
              "3fs",
              "עָדֶיהָ",
              "preposition 'until/as far as' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-ad-sfx2ms",
            "cells": [
              "2ms",
              "עָדֶיךָ",
              "preposition 'until/as far as' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-ad-sfx2fs",
            "cells": [
              "2fs",
              "עָדַיִךְ",
              "preposition 'until/as far as' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-ad-sfx1cs",
            "cells": [
              "1cs",
              "עָדַי",
              "preposition 'until/as far as' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-ad-sfx3mp",
            "cells": [
              "3mp",
              "עֲדֵיהֶם",
              "preposition 'until/as far as' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-ad-sfx3fp",
            "cells": [
              "3fp",
              "עֲדֵיהֶן",
              "preposition 'until/as far as' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-ad-sfx2mp",
            "cells": [
              "2mp",
              "עֲדֵיכֶם",
              "preposition 'until/as far as' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-ad-sfx2fp",
            "cells": [
              "2fp",
              "עֲדֵיכֶן",
              "preposition 'until/as far as' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-ad-sfx1cp",
            "cells": [
              "1cp",
              "עָדֵינוּ",
              "preposition 'until/as far as' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "appendix": "B",
            "page": "a-14"
          }
        ],
        "appendixOnly": true
      },
      {
        "id": "particle-prep-attached-tachat",
        "title": "Preposition תַּחַת ('under') with attached pronouns",
        "columns": [
          "Suffix",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "particle-prep-attached-tachat-sfx3ms",
            "cells": [
              "3ms",
              "תַּחְתָּיו",
              "preposition 'under' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-tachat-sfx3fs",
            "cells": [
              "3fs",
              "תַּחְתֶּיהָ",
              "preposition 'under' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-tachat-sfx2ms",
            "cells": [
              "2ms",
              "תַּחְתֶּיךָ",
              "preposition 'under' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-tachat-sfx2fs",
            "cells": [
              "2fs",
              "תַּחְתַּיִךְ",
              "preposition 'under' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-tachat-sfx1cs",
            "cells": [
              "1cs",
              "תַּחְתַּי",
              "preposition 'under' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-tachat-sfx3mp",
            "cells": [
              "3mp",
              "תַּחְתֵּיהֶם",
              "preposition 'under' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-tachat-sfx3fp",
            "cells": [
              "3fp",
              "תַּחְתֵּיהֶן",
              "preposition 'under' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-tachat-sfx2mp",
            "cells": [
              "2mp",
              "תַּחְתֵּיכֶם",
              "preposition 'under' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-tachat-sfx2fp",
            "cells": [
              "2fp",
              "תַּחְתֵּיכֶן",
              "preposition 'under' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-tachat-sfx1cp",
            "cells": [
              "1cp",
              "תַּחְתֵּינוּ",
              "preposition 'under' with attached pronoun"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "appendix": "B",
            "page": "a-14"
          }
        ],
        "appendixOnly": true
      },
      {
        "id": "particle-prep-attached-kmo",
        "title": "כְּמוֹ/כְּ (like, as) + attached pronoun",
        "columns": [
          "Suffix",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "particle-prep-attached-kmo-3ms",
            "cells": [
              "3ms",
              "כָּמֹהוּ",
              "like him/it"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-kmo-3fs",
            "cells": [
              "3fs",
              "כָּמוֹהָ",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-kmo-2ms",
            "cells": [
              "2ms",
              "כָּמוֹךָ",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-kmo-2fs",
            "cells": [
              "2fs",
              "כָּמוֹךְ",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-kmo-1cs",
            "cells": [
              "1cs",
              "כָּמוֹנִי",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-kmo-3mp",
            "cells": [
              "3mp",
              "כָּהֶם",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-kmo-3fp",
            "cells": [
              "3fp",
              "כָּהֶן",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-kmo-2mp",
            "cells": [
              "2mp",
              "כָּכֶם",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-kmo-2fp",
            "cells": [
              "2fp",
              "כָּכֶן",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-kmo-1cp",
            "cells": [
              "1cp",
              "כָּמוֹנוּ",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "appendix": "B",
            "page": "a-13-a-14",
            "pageCount": 2
          }
        ],
        "appendixOnly": true
      },
      {
        "id": "particle-prep-attached-mn",
        "title": "מִן (from, than) + attached pronoun",
        "columns": [
          "Suffix",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "particle-prep-attached-mn-3ms",
            "cells": [
              "3ms",
              "מִמֶּנּוּ",
              "from him/it"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-mn-3fs",
            "cells": [
              "3fs",
              "מִמֶּנָּה",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-mn-2ms",
            "cells": [
              "2ms",
              "מִמְּךָ",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-mn-2fs",
            "cells": [
              "2fs",
              "מִמֵּךְ",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-mn-1cs",
            "cells": [
              "1cs",
              "מִמֶּנִּי",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-mn-3mp",
            "cells": [
              "3mp",
              "מֵהֶם",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-mn-3fp",
            "cells": [
              "3fp",
              "מֵהֶן",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-mn-2mp",
            "cells": [
              "2mp",
              "מִכֶּם",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-mn-2fp",
            "cells": [
              "2fp",
              "מִכֶּן",
              ""
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-prep-attached-mn-1cp",
            "cells": [
              "1cp",
              "מִמֶּנּוּ",
              "from us"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": "printed identically to the 3ms form 'from him'; only the surrounding context distinguishes 'from us' from 'from him'"
          }
        ],
        "sourceRef": [
          {
            "appendix": "B",
            "page": "a-13-a-14",
            "pageCount": 2
          }
        ],
        "appendixOnly": true
      },
      {
        "id": "particle-object-marker",
        "title": "Direct object marker אֵת/אוֹת with attached pronouns",
        "columns": [
          "Suffix",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "particle-object-marker-sfx3ms",
            "cells": [
              "3ms",
              "אוֹתוֹ",
              "direct-object marker אֵת/אֶת with attached pronoun (accusative 'him/her/you/me/etc.')"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-object-marker-sfx3fs",
            "cells": [
              "3fs",
              "אֹתָהּ",
              "direct-object marker אֵת/אֶת with attached pronoun (accusative 'him/her/you/me/etc.')"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-object-marker-sfx2ms",
            "cells": [
              "2ms",
              "אֹתְךָ",
              "direct-object marker אֵת/אֶת with attached pronoun (accusative 'him/her/you/me/etc.')"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-object-marker-sfx2fs",
            "cells": [
              "2fs",
              "אֹתָךְ",
              "direct-object marker אֵת/אֶת with attached pronoun (accusative 'him/her/you/me/etc.')"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-object-marker-sfx1cs",
            "cells": [
              "1cs",
              "אֹתִי",
              "direct-object marker אֵת/אֶת with attached pronoun (accusative 'him/her/you/me/etc.')"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-object-marker-sfx3mp",
            "cells": [
              "3mp",
              "אוֹתָם",
              "direct-object marker אֵת/אֶת with attached pronoun (accusative 'him/her/you/me/etc.')"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-object-marker-sfx3fp",
            "cells": [
              "3fp",
              "אֶתְהֶן",
              "direct-object marker אֵת/אֶת with attached pronoun (accusative 'him/her/you/me/etc.')"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-object-marker-sfx2mp",
            "cells": [
              "2mp",
              "אֶתְכֶם",
              "direct-object marker אֵת/אֶת with attached pronoun (accusative 'him/her/you/me/etc.')"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-object-marker-sfx2fp",
            "cells": [
              "2fp",
              "אֶתְכֶן",
              "direct-object marker אֵת/אֶת with attached pronoun (accusative 'him/her/you/me/etc.')"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "particle-object-marker-sfx1cp",
            "cells": [
              "1cp",
              "אוֹתָנוּ",
              "direct-object marker אֵת/אֶת with attached pronoun (accusative 'him/her/you/me/etc.')"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "appendix": "B",
            "page": "a-13-a-14",
            "pageCount": 2
          }
        ],
        "appendixOnly": true
      },
      {
        "id": "verb-pual-appendix",
        "title": "Pual — the only two attested forms printed in the book",
        "columns": [
          "Suffix",
          "Hebrew",
          "Gloss / note"
        ],
        "rows": [
          {
            "id": "verb-pual-appendix-inf-1cs-ana",
            "cells": [
              "1cs",
              "עֻנּוֹתִי",
              "sole printed Pual Infinitive citation (Ps 132:1); appendix says Imperative and 1st-person Jussive never occur in Pual"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          },
          {
            "id": "verb-pual-appendix-advinf-gnb",
            "cells": [
              "",
              "גֻּנַּב",
              "sole printed Pual Adverbial Infinitive citation (Gen 40:15)"
            ],
            "introducedLesson": null,
            "appendixOnly": true,
            "ambiguityNote": null
          }
        ],
        "sourceRef": [
          {
            "appendix": "C",
            "page": "a-17"
          }
        ],
        "appendixOnly": true
      }
    ]
  };
})();
