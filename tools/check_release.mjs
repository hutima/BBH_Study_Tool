...
//      js/data/bbh_reader.js exactly 98 passages (task #24 stage B: 86
//      pre-existing + 12 new prose selections across Josh/1Kgs/2Kgs/Esth —
//      was 86 after task 13a: 78 pre-existing + 8 new challenge-tier
//      selections; was 78 after the Reader book expansion: 52 Genesis + 26
//      across Ruth/Jonah/Exod/Deut/Judg/1Sam/2Sam), js/data/bbh_alphabet.js
...
// check5 above).
{
  ...
  if (!existsSync(path.join(ROOT, readerPath))) {
    ...
    if (passageCount !== 98) fail(`check5b: expected 98 passages registered in ${readerPath}, found ${passageCount}`);
    else report(`check5b: 98 passages registered in bbh_reader.js (pass)`);
  }
